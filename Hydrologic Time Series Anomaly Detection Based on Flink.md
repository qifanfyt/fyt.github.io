...
**答:K-Means++算法**模型训练完成后，可以得到离散的状态序列{T1,T2,...}，并且通过将矩阵转换为数据帧计算获得状态转移矩阵。再将ARIMA模型得到的可疑异常点集及其前一时刻的值输入到**聚类**训练好的模型中，以获取异常状态及其前一时刻状态，再通过状态转换数据帧估计异常的值及其前一时刻的值，输出真实异常值的概率。
